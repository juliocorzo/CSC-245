...
// Step 4: Private and public postOrder() methods.
...
// Step 6: Private and public height() methods.
template <class Comparable>
...
// Step 8: Private and public numLeaves() methods.
template<class Comparable>
...
int BinarySearchTree<Comparable>::numLeaves (BinaryNode<Comparable> *t) const {
  if(t == NULL) return 0;
  else return (1 + numLeaves(t -> left) + numLeaves(t -> right));
}

// Step 9 part 1: Private and public isBalanced methods.
template<class Comparable>
bool BinarySearchTree<Comparable>::isBalanced() const {
  if(isEmpty()) return 1;
  else isBalanced(root);
}
...
bool BinarySearchTree<Comparable>::isBalanced(BinaryNode <Comparable> *t) const {
  ...
  int heightLeft = height(t -> left);
  int heightRight = height(t -> right);
  if(abs(heightLeft - heightRight) <= 1 && isBalanced(t -> left) && isBalanced(t -> right))
    return 1;
  return 0;
}

// Methods included with the assignment
template<class Comparable>
